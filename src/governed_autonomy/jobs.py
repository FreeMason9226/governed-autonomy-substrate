...
class SQLiteJobStore:
    ...
    def claim(self) -> Job | None:
        ...
        if row is None:
            return None
        self.db.execute(
            "UPDATE jobs SET status='running', attempts=attempts+1 WHERE job_id=? AND status='queued'",
            (row[0],),
        )
        self.db.commit()
    ...
    def complete(self, job_id: str) -> None:
        self.db.execute(
            "UPDATE jobs SET status='succeeded' WHERE job_id=? AND status='running'", (job_id,)
        )
        self.db.commit()
    ...
    def fail(self, job_id: str, error: str, *, retry_delay: float = 1.0) -> None:
        ...
        self.db.execute(
            "UPDATE jobs SET status=?, last_error=?, available_at=? WHERE job_id=?",
